...
class SessionPanel {
    ...
    static show(server, context) {
        ...
        const panel = vscode.window.createWebviewPanel('co-dev.sessions', 'Co-Dev Sessions', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    _subscribeWebViewMessages() {
        this.panel.webview.onDidReceiveMessage((msg) => {
            if (msg.type === 'send') {
                ...
                this._addFeedItem(msg.sessionId, 'status', `[Permission] ${msg.decision === 'allow' ? '✓ 승인' : '✗ 거절'} (${msg.requestId})`);
                this._pushFeedUpdate(msg.sessionId);
            }
            else if (msg.type === 'ready') {
                for (const [id] of this.feeds) {
                    this._pushFeedUpdate(id);
                }
            }
        });
    }
    ...
    _buildHtml() {
        // 패널 열 때 이미 실행 중인 세션 카드를 초기 HTML에 포함
        const existingSessions = this.server.getSessions();
        const initialCardsHtml = existingSessions.length === 0
            ...
            : existingSessions.map(h => this._buildCardHtml(h.id, h.roleName, h.status, h.startedAt.toLocaleTimeString())).join('');
...
<script>
  ...
  // 기존 message handler에 control-request 추가
  window.addEventListener('message', e => {
    const msg = e.data;
